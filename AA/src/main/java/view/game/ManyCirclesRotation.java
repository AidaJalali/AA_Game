...
public class ManyCirclesRotation extends Application {
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        // Create a timeline that rotates the circles around the center
        Timeline timeline = new Timeline(new KeyFrame(Duration.millis(50), event -> {
            double angle = (circlesGroup.getRotate() + 1) % 360;
            double x = CENTER_X + RADIUS * Math.cos(Math.toRadians(angle));
            double y = CENTER_Y + RADIUS * Math.sin(Math.toRadians(angle));
            circlesGroup.setTranslateX(x);
            circlesGroup.setTranslateY(y);
